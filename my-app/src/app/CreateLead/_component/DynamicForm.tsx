...
const DynamicForm = forwardRef<HTMLFormElement, DynamicFormProps>(({ schema }, ref) => {
    ...
    const handleSelect = (ranges: RangeKeyDict, fieldName: string) => {
        ...
        if (startDate && endDate) {
            ...
            const rangeAsRecord: Record<number, string> = {
                0: format(startDate, 'yyyy-MM-dd'),
                1: format(endDate, 'yyyy-MM-dd'),
            };
            handleChange(fieldName, rangeAsRecord);
        }
    };
    ...
    const renderField = (field: FieldSchema) => {
        switch (field.type) {
            ...
            case 'rangedate':
                return (
                    <div className="p-4 border-yellow-100 b rounded-md shadow-md  w-fit">
                        <DateRangePicker
                            ranges={[selectionRange]}
                            onChange={(ranges) => handleSelect(ranges, field.name)}
                            locale={enUS}
                            months={1}
                            direction={'vertical'}
                            // staticRanges={[]}
